...
const brand = ({brands}) => {
    ...
    const head = () => (
        <Head>
            ...
            <link rel="canonical" href={`${DOMAIN}/dealer/brand`} />
            ...
            <meta property="og:url" content={`${DOMAIN}/dealer/brand`} />
  ...
  if (isAuth() && isAuth().role === 'Customer'){
    ...
    return (
        <React.Fragment>
          {head()}
            <Dashboard>
                <div className="canvas" style={{height: "250px"}}>
                        <h1 className="heading1">Brand Management</h1>
                        <p className="lead text-white font-weight-bold"><strong>List of Brands made till now</strong></p>
                </div><br/>
...
brand.getInitialProps = () => {
  
  const token = getCookie('token');
  return ListBrands(token).then(data => {
    if (data.error){
      console.log(error);
    }else{
      return {
